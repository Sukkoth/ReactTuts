...
const EditTransaction = () => {
    ...
    const [transaction, setTransaction] = useState({
        ...
        date: transactionFetched?.data?.createdAt,
        category: transactionFetched?.data?.category,
        notes: transactionFetched?.data?.notes,
    });

    //---Destructuring---
    const { name, amount, transactionType, date, category, notes } =
        transaction;
    //---onchange handler----
    const onChange = (e) => {
            ...
            <div className='container px-4 mx-auto'>
                <div className='max-w-md mx-auto text-center'>
                    ...
                    <p className='mb-12 text-lg font-medium leading-normal text-gray-600'>
                        You are updatting transaction to .....{' '}
                        {transactionFetched?.data?.name}
                    </p>
                    ...
                    <form onSubmit={onSubmit}>
                        ...
                        {loading ? (
                            <button
                                disabled
                                type='submit'
                                className='w-full py-4 mb-8 font-semibold text-white transition duration-200 ease-in-out bg-gray-600 border border-gray-700 px-9 rounded-xl shadow-4xl focus:ring focus:ring-indigo-300 hover:bg-indigo-700'
                            >
                                Loading . . .
                            </button>
                        ) : (
